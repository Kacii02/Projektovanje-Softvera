 ...
 * @author kacan
 */
public class Smena implements Serializable, domain.DomainObject<Smena> {
    ...
    public String getInsertQuery() {
        return "INSERT INTO smena VALUES(?,?,?) "
                + "WHERE idSmena=?";
    }
    ...
    public String getUpdateQuery() {
        return "UPDATE smena SET naziv=?, pocetak=?, kraj=? "
                + "WHERE idSmena=?";
    }
    ...
    public Smena createFromResultSet(ResultSet rs) throws SQLException {
         if (rs.next()) {
            ...
            return new Smena(rsIdSmena, rsNaziv, rsPocetak, rsKraj);
        }
        return null;
    }
    ...
    public List<Smena> createListFromResultSet(ResultSet rs) throws SQLException {
        ...
        while(rs.next()) {
            ...
            LocalTime rsPocetak = rs.getTime("pocetak").toLocalTime();
            LocalTime rsKraj = rs.getTime("kraj").toLocalTime();
            lista.add(new Smena(rsIdSmena, rsNaziv, rsPocetak, rsKraj));
        }
        return lista;
    }
